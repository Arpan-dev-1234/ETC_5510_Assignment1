...
knitr::opts_chunk$set(echo = F, cache = FALSE, cache.lazy = FALSE )
```
...
The number of variables are `r ncol(crime_data)` (1pt) and the number of rows are `r nrow(crime_data)` (1pt)
...
ggplot(comparison_data, aes(x = Year,    # 1pt
  ...
  xlab("Years") +   # 1pt
ylab("Incidents") +   # 1pt
  ggtitle("Number of criminal incidents") +
  theme(plot.title = element_text(hjust = 0.5)) # 1pt
